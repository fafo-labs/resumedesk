...
export function LoginForm() {
  ...
  function handleLogin() {
    login({ email, password }).then((data) => {
      if (data?.error && data.code !== "email_not_confirmed") {
        toast({ description: "Please confirm your email or sign up", variant: 'destructive' });
      }
      if(data?.error) {
  ...
  return (
    <form action={'#'} className={'px-6 md:px-16 pb-6 py-8 gap-6 flex flex-col items-center justify-center'}>
      <Link href='/' className="text-2xl font-semibold --local-clashgrotesk">ResumeDesk</Link>
